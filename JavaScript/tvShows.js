const tvContainer = document.querySelector(".container-List");
let list = [];
let currentPage = 1;

const getData = async(page) => {
    const ress = await fetch(
        `https://api.themoviedb.org/3/tv/popular?api_key=8dbe9b2f66b85a085ad28e12a7c2b943&language=es-MX&page=${page}`
    );
...
const insertCart = (item, i) => {
    ...
    <div class="defaultCard" id={card${i}}>
        ...
        <div>
            ...
            <span id='cardDate'>${item.first_air_date}</span>
        </div>
    </div>
    `;
};
    ...
    getData(currentPage).then((res) => {
        ...
        res.results.forEach((item, i) => {
            if (item.poster_path) {
                ...
                tvContainer.innerHTML += card;
            }
        });
        list = tvContainer.querySelectorAll(".defaultCard");
